...
public class NonLinearEq {
	...
	public int solveNewton(double init_x) {
		...
		if(n == maxlter || Double.isNaN(x)) {
			...
			if(n != 30) {	
				throw new ArithmeticException("Newton method didn't converge after " + n + " iteration.");
			}
			return(0);
		}
		else {
	...
	public static void main(String [] args) {
		...
		try{
			...
			for(int i = 0; i < args.length; i++) {
				...
				// count number of exception
				try{
				if(X.solveNewton(init_x[i]) == 0) {
					//count++;	
					throw new ArithmeticException("Newton method didn't converge after 30 iteration.");
				}
